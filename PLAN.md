...
- **Workload Items**: Use one display-named 1Password item per workload. The cluster vault carries scope; tag Kubernetes-created items only with `Kubelab`. Discover item ownership, fields, and websites from External Secrets and Gateway API resources. Present every item associated with an HTTPRoute as a Login item and every unrouted item as a Server item. Preserve non-empty values edited in 1Password, generate only declared internal credentials, and archive Kubernetes-owned items with no remaining declarative consumer only while the cluster `apps` reconciliation is ready and has applied the current Git source revision. Keep the annotation contract to constants, defaults, and generated fields. Use native username and password fields only for functional browser logins. Name custom fields by grouping first and sort them by major group, with `database-*` before `api-key-*`, credential usernames before their passwords, then labels alphabetically.
